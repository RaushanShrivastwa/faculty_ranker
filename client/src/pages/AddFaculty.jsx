...
// Clamp numerical input between 0 and 5
const clamp = (value) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.image instanceof File) {
        ...
        const uploadRes = await axios.post('/api/faculty/upload', imageData, {
          headers: {
      ...
      if (err.response?.status === 409) setError('Faculty already exists.');
      ...
      <div className="max-w-md mx-auto bg-gray-800 rounded-xl shadow-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isDisabled}
            className={`w-full py-2 px-4 rounded text-white ${isDisabled ? 'bg-gray-500' : 'bg-purple-600 hover:bg-purple-700'}`}
          >
            {isUploading ? 'Uploading...' : isSubmitting ? 'Submitting...' : 'Add Faculty'}
          </button>
        </form>
      </div>
    </div>
